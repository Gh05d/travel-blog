...
const files = fs.readdirSync(ARTICLES_DIR, { withFileTypes: true })          :contentReference[oaicite:6]{index=6}
  .filter(dirent => dirent.isFile() && dirent.name.endsWith('.html'))
  .map(dirent => dirent.name);

const entries = files.map(filename => {
  const fullPath = path.join(ARTICLES_DIR, filename);
  const html     = fs.readFileSync(fullPath, 'utf8');                       :contentReference[oaicite:7]{index=7}
